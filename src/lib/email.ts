...
const baseUrl = isDev() ? "http://localhost:3000" : "https://voltpassword.xyz";
...
export const verificationEmailTemplate = (token: string) => {
  return {
    ...
    <p style="font-size: 16px">
      <a href="${baseUrl}/verify?token=${token}">${baseUrl}/verify?token=${token}</a>
    </p>
...
export const passwordResetTemplate = (token: string) => {
  return {
    ...
    <p style="font-size: 16px">
      <a href="${baseUrl}/recover/reset?token=${token}">${baseUrl}/recover/reset?token=${token}</a>
    </p>
